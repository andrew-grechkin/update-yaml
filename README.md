...
- `UPDATE_YAML_MAX_LINE_WIDTH` - opt-in fold budget for `>` block form. Set to a positive integer to enable folding at
  that column; unset, zero or non-positive keeps folding disabled
...
- Data-provided inline comments override existing comments in output
- Preserves key order of the original document; keys added by data land at their alphabetical position when the
  surrounding siblings are already sorted, otherwise appended at the end in data provided order
...
- Values that data quoted but that don't need quoting are unquoted on emission
- Long single-line plain values can be folded to `>` block form; opt in via `UPDATE_YAML_MAX_LINE_WIDTH`
...
plain or block scalar - and that style flows through to the output.

The one adjustment: values that data quoted but that would round-trip fine as plain scalars get their quotes dropped.

Numeric-looking strings (`'1982'`), reserved words (`'true'`, `'null'`, `'~'`), and YAML 1.1 boolean spellings (`'yes'`,
`'On'`, `'off'`) stay quoted to preserve semantics for downstream parsers.

Long single-line plain values can be folded to `>` block form (opt-in via `UPDATE_YAML_MAX_LINE_WIDTH`):

```bash
UPDATE_YAML_MAX_LINE_WIDTH=120 update-yaml <<< 'k: v' <(echo 'k: This is a very long single-line description that clearly exceeds the line width and should be broken into multiple lines to improve readability of the YAML file')
```

### Data-provided inline comments override source

If data has an inline comment on a leaf key, it replaces the source's on that key.
Source's comment is preserved when data doesn't provide one.
Comments on branch (mapping) keys are never overridden - the tool doesn't rewrite non-leaf keys.
...
# data
service:  # ignored - service is not a leaf
